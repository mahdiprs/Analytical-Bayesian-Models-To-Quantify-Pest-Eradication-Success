# function for the theory paper using symbolic algebra
library(calculus)
...
pgfM1firstDTC <- function(t,delta,rho,s){
  t1 <- t - 1
  t2 <- pgfM1(t1, delta, rho, s)
  t5 <- pgfM1(t1, delta, rho, s * (1 - delta))
  return((t2 - t5) / (1 - t5))
}

# Mean population size Model 1
MeanPopulationM1 <- function(t,delta,rho){
  f <- function(s) pgfM1(t,delta,rho, s)
...
VarPopulationM1 <- function(t,delta,rho){
  f <- function(s) pgfM1(t,delta,rho, s)
  val = derivative(f = f, var = c(s=1), order = 2)+MeanPopulationM1(t,delta,rho)-MeanPopulationM1(t,delta,rho)^2
  return(val)
}

# Probability of no detection Model 1
P0M1 <- function(t,delta,rho) {
  val = pgfM1(t, delta, rho, 0)
  return(val)
}
...
ProbNotDCTUptoTM1 <- function(t,delta,rho){
  if (t == 0){
    return(pgfInitial(rho,1-delta)) # exp(-delta * rho)
    }else{
  return(PNotDCTCondUptMinusOneM1(t, delta, rho) * ProbNotDCTUptoTM1(t - 1, delta, rho))}
}

# Probability of  P(+Dt|~Dt-1,~Dt-2,...~D0)
PFirstDCTCondUptMinusOneNotDCTM1 <- function(t,delta,rho){
  t1 <- PNotDCTCondUptMinusOneM1(t, delta, rho)
  return(1 - t1)
}
...
PFirstDCTwithUptMinusOneNotDCTM1 <- function(t,delta,rho){
  if (t == 0){
    return(1 - pgfInitial(rho,1-delta)) # 1- exp(-delta * rho)
  }else{
    return(PFirstDCTCondUptMinusOneNotDCTM1(t, delta, rho) * ProbNotDCTUptoTM1(t - 1, delta, rho))
  }
}
...
pgfM2 <- function(t1,t2,delta,lambda,rho,s){
  ...
  c2 <- t1 - 1 - t2
  if (!(c1 != 0 || t2 != 0 || t1 != 0))
    return(pgfInitial(rho, s * pgfDCT(delta, 0)) / pgfInitial(rho, pgfDCT(delta, 0)))
  else if (c2 == 0)
    return(pgfM2(t1 - 1, t2, delta, lambda, rho, phi_x(lambda, s)))
  else
    return(pgfM2(t1, t2 - 1, delta, lambda, rho, s * pgfDCT(delta, 0)) / pgfM2(t1, t2 - 1, delta, lambda, rho, pgfDCT(delta, 0)))
}


# Mean population size Model 2
MeanPopulationM2 <- function(t1,t2,delta,lambda,rho){
  f <- function(s) pgfM2(t1,t2,delta,lambda,rho,s)
  val = derivative(f = f, var = c(s=1), order = 1)
    return(val)
}
...
VarPopulationM2 <- function(t1,t2,delta,lambda,rho){
  f <- function(s) pgfM2(t1,t2,delta,lambda,rho,s)
  val = derivative(f = f, var = c(s=1), order = 2)+MeanPopulationM2(t1,t2,delta,lambda,rho)-MeanPopulationM2(t1,t2,delta,lambda,rho)^2
  return(val)
}

# Probability of no detection Model 2
P0M2 <- function(t1,t2,delta,lambda,rho) {
  val = pgfM2(t1,t2,delta,lambda,rho, 0)
  return(val)
}

# pgf population size conditional on first detection after t-1 negative survey
pgfPopSzConFrsDCTM2 <- function(t,delta,lambda,rho,s){
  if (t == 0)
    return((pgfInitial(rho, s) - pgfInitial(rho, s * pgfDCT(delta, 0))) / (1 - pgfInitial(rho, pgfDCT(delta, 0))))
  else
    return((pgfM2(t, t - 1, delta, lambda, rho, s) - pgfM2(t, t - 1, delta, lambda, rho, s * pgfDCT(delta, 0))) / (1 - pgfM2(t, t - 1, delta, lambda, rho, pgfDCT(delta, 0))))
...
PNotDCTCondUptMinusOneM2 <- function(t,delta,lambda,rho) {
  return(pgfNbrDCTM2(t, delta, lambda, rho, 0))
}

#  Probability of P(~D0,~D1,...,~Dt) = p(~Dt|~Dt-1,~Dt-2,...~D0) * p(~Dt-1,~Dt-2,...~D0)
ProbNotDCTUptoTM2 <- function(t,delta,lambda,rho){
  if (t == 0)
    return(pgfInitial(rho, pgfDCT(delta, 0)))
  else
...
PFirstDCTwithUptMinusOneNotDCTM2 <- function(t,delta,lambda,rho){
  if (t == 0)
    return(1 - pgfInitial(rho, pgfDCT(delta, 0)))
  else
    return(PFirstDCTCondUptMinusOneNotDCTM2(t, delta, lambda, rho) * ProbNotDCTUptoTM2(t - 1, delta, lambda, rho))

}

# Probability not detecting upto and including time t conditiong on presence P(~Dt,~Dt-1,~Dt-2,...~D0|Xt>0)
